...
class QueryPool {
  public:
    [[nodiscard]] static Result<QueryPool> create(const Device& device, VkQueryType type,
                                                  std::uint32_t count);
    ...
    [[nodiscard]] VkQueryPool native() const {
        return pool_;
    }
    [[nodiscard]] VkQueryPool vkQueryPool() const {
        return native();
    }
    [[nodiscard]] VkQueryType type() const {
        return type_;
    }
    ...
    getResults(std::uint32_t first, std::uint32_t resultCount, VkQueryResultFlags flags = 0) const;
