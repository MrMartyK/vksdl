...
class Device {
  public:
    ...
    [[nodiscard]] VkDevice native() const {
        return device_;
    }
    ...
    [[nodiscard]] bool hasDedicatedTransfer() const {
        return families_.hasDedicatedTransfer();
    }
    [[nodiscard]] bool hasAsyncCompute() const {
        return families_.hasDedicatedCompute();
    }
    [[nodiscard]] bool hasDedicatedCompute() const {
        return families_.hasDedicatedCompute();
    }
    [[nodiscard]] VkDeviceSize minUniformBufferOffsetAlignment() const {
        return minUboAlignment_;
    }
    [[nodiscard]] VkSampleCountFlagBits maxMsaaSamples() const {
        return maxMsaaSamples_;
    }
    [[nodiscard]] float timestampPeriod() const {
        return timestampPeriod_;
    }
    [[nodiscard]] const char* gpuName() const {
        return gpuName_.c_str();
    }

    // RT properties (0 when RT not requested)
    [[nodiscard]] std::uint32_t shaderGroupHandleSize() const {
        return rtHandleSize_;
    }
    [[nodiscard]] std::uint32_t shaderGroupBaseAlignment() const {
        return rtBaseAlignment_;
    }
    [[nodiscard]] std::uint32_t shaderGroupHandleAlignment() const {
        return rtHandleAlignment_;
    }
    [[nodiscard]] std::uint32_t maxRayRecursionDepth() const {
        return rtMaxRecursion_;
    }
    [[nodiscard]] VkDeviceSize minAccelerationStructureScratchOffsetAlignment() const {
        return rtScratchAlignment_;
    }

    [[nodiscard]] PFN_vkCmdTraceRaysKHR traceRaysFn() const {
        return pfnTraceRays_;
    }

    // VK_EXT_device_fault support (opportunistic -- always safe to call)
    [[nodiscard]] bool hasDeviceFault() const {
        return hasDeviceFault_;
    }
    ...
    [[nodiscard]] bool hasMemoryPriority() const {
        return hasMemoryPriority_;
    }
    ...
    [[nodiscard]] bool hasUnifiedImageLayouts() const {
        return hasUnifiedLayouts_;
    }
    ...
    [[nodiscard]] bool hasGPL() const {
        return hasGPL_;
    }
    [[nodiscard]] bool hasGplFastLinking() const {
        return hasGplFastLinking_;
    }
    [[nodiscard]] bool hasGplIndependentInterpolation() const {
        return hasGplIndepInterp_;
    }
    ...
    [[nodiscard]] bool hasPipelineCreationCacheControl() const {
        return hasPCCC_;
    }
    ...
    [[nodiscard]] bool hasPushDescriptors() const {
        return hasPushDescriptors_;
    }
    ...
    [[nodiscard]] bool hasInvocationReorder() const {
        return hasInvocationReorder_;
    }
    ...
    [[nodiscard]] bool hasPipelineBinary() const {
        return hasPipelineBinary_;
    }
    ...
    [[nodiscard]] bool hasPresentTiming() const {
        return hasPresentTiming_;
    }
    [[nodiscard]] bool hasGoogleDisplayTiming() const {
        return hasGoogleDisplayTiming_;
    }
    [[nodiscard]] bool hasExtPresentTiming() const {
        return hasExtPresentTiming_;
    }

    // VK_EXT_mesh_shader support.
    // When true, MeshPipelineBuilder is usable and drawMeshTasksFn() is valid.
    [[nodiscard]] bool hasMeshShaders() const {
        return hasMeshShaders_;
    }
    [[nodiscard]] PFN_vkCmdDrawMeshTasksEXT drawMeshTasksFn() const {
        return pfnDrawMeshTasks_;
    }
...
class DeviceBuilder {
  public:
    ...
    DeviceBuilder&
    requireFeatures(std::function<void(VkPhysicalDeviceVulkan13Features&)> configureFn);
    DeviceBuilder&
    requireFeatures12(std::function<void(VkPhysicalDeviceVulkan12Features&)> configureFn);
