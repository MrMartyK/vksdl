...
class BindlessTable {
  public:
    [[nodiscard]] static Result<BindlessTable>
    create(const Device& device, std::uint32_t capacity,
    ...
    void writeImage(std::uint32_t index, VkImageView view, VkImageLayout layout, VkSampler sampler);
    void writeStorageImage(std::uint32_t index, VkImageView view, VkImageLayout layout);
    void writeBuffer(std::uint32_t index, VkBuffer buffer, VkDeviceSize size,
                     VkDeviceSize offset = 0);

    // Bind this table to a command buffer at the given set index.
    void bind(VkCommandBuffer cmd, VkPipelineBindPoint bindPoint, VkPipelineLayout pipelineLayout,
              std::uint32_t set = 0) const;
    ...
    [[nodiscard]] VkDescriptorType descriptorType() const {
        return type_;
    }
