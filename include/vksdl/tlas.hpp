...
class Tlas {
  public:
    ...
    [[nodiscard]] bool supportsUpdate() const {
        return allowUpdate_;
    }
    [[nodiscard]] std::uint32_t maxInstanceCount() const {
        return maxInstanceCount_;
    }
    [[nodiscard]] VkDeviceSize updateScratchSize() const {
        return updateScratchSize_;
    }
    ...
    [[nodiscard]] Result<void> cmdUpdate(VkCommandBuffer cmd, const Buffer& scratch,
...
class TlasBuilder {
  public:
    ...
    TlasBuilder& addInstance(const Blas& blas, const float transform[3][4],
                             std::uint32_t customIndex = 0, std::uint8_t mask = 0xFF,
                             std::uint32_t sbtOffset = 0);
    ...
    [[nodiscard]] Result<Tlas> cmdBuild(VkCommandBuffer cmd, const Buffer& scratch,
                                        const Buffer& instanceBuffer, std::uint32_t instanceCount);
