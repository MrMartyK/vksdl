...
class OrbitCamera {
  public:
    OrbitCamera(float targetX, float targetY, float targetZ, float distance, float yaw = 0.0f,
                float pitch = 0.0f);
    ...
    void setViewport(std::uint32_t w, std::uint32_t h) {
        vpW_ = w;
        vpH_ = h;
    }
    ...
    [[nodiscard]] const float* target() const {
        return tgt_;
    }
