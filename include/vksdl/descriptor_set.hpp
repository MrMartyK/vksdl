...
class DescriptorSet {
  public:
    ...
    [[nodiscard]] VkDescriptorSet vkDescriptorSet() const {
        return native();
    }
    [[nodiscard]] VkDescriptorSetLayout vkDescriptorSetLayout() const {
        return layout_;
    }
    [[nodiscard]] VkDescriptorPool vkDescriptorPool() const {
        return pool_;
    }

    void updateBuffer(std::uint32_t binding, VkBuffer buffer, VkDeviceSize size,
                      VkDeviceSize offset = 0);

    void updateImage(std::uint32_t binding, VkImageView view, VkImageLayout layout,
                     VkSampler sampler = VK_NULL_HANDLE);

    void updateAccelerationStructure(std::uint32_t binding, VkAccelerationStructureKHR as);
...
class DescriptorSetBuilder {
  public:
    ...
    DescriptorSetBuilder& addUniformBuffer(std::uint32_t binding, VkShaderStageFlags stageFlags);
    DescriptorSetBuilder& addDynamicUniformBuffer(std::uint32_t binding,
                                                  VkShaderStageFlags stageFlags);
    DescriptorSetBuilder& addStorageBuffer(std::uint32_t binding, VkShaderStageFlags stageFlags);
    DescriptorSetBuilder& addStorageImage(std::uint32_t binding, VkShaderStageFlags stageFlags);
    ...
    DescriptorSetBuilder& addBinding(std::uint32_t binding, VkDescriptorType type,
