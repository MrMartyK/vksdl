...
[[nodiscard]] inline Mat4 perspectiveVk(float fovY, float aspect,
...
// Pair with: depth clear = 0.0, compare op = GREATER_OR_EQUAL, D32_SFLOAT.
[[nodiscard]] inline Mat4 perspectiveInfiniteReverseZ(float fovY, float aspect,
                                                       float zNear) {
    const float f = 1.0f / std::tan(fovY * 0.5f);
...
[[nodiscard]] inline Mat4 perspectiveForwardZ(float fovY, float aspect,
                                              float zNear, float zFar) {
    const float f = 1.0f / std::tan(fovY * 0.5f);
...
[[nodiscard]] inline Mat4 lookAt(const float* eye, const float* target,
                                 const float* worldUp) {
