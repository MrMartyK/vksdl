...
static const char* cullName(VkCullModeFlags m) {
    ...
    case VK_CULL_MODE_FRONT_AND_BACK: return "front+back";
    default:                          return "?";
    }
}
...
static const char* topoName(VkPrimitiveTopology t) {
    switch (t) {
    case VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST: return "triangles";
    case VK_PRIMITIVE_TOPOLOGY_LINE_LIST:     return "lines";
    case VK_PRIMITIVE_TOPOLOGY_POINT_LIST:    return "points";
    default:                                  return "?";
    }
}
...
int main() {
    ...
    auto vertexBuffer = vksdl::uploadVertexBuffer(
        allocator, device, cubeVerts, sizeof(cubeVerts)).value();
    ...
    auto updateTitle = [&]() {
        ...
        std::snprintf(buf, sizeof(buf),
            ...
            faceName(frontFace),
            topoName(kTopologies[topoIdx]),
            depthTest ? "on" : "off");
        SDL_SetWindowTitle(window.sdlWindow(), buf);
    };
    updateTitle();

    std::printf("Controls: 1/2 cull mode, 3/4 front face, 5/6 topology, 7/8 depth test, ESC quit\n");
