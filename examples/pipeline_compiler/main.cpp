...
static constexpr int TOTAL_PERMS =
    SHADER_VARIANTS * CULL_MODES * BLEND_MODES * POLY_MODES * DEPTH_CMP_MODES * PUSH_MODES; // 128
...
static void pushQuad(std::vector<OverlayVertex>& v, float x0, float y0, float x1, float y1, float r,
...
static const char* modeName(Mode m) {
    ...
    case Mode::AutoWarm:
        return "Auto (warm)";
    case Mode::AutoWarmStats:
        return "Auto (warm)+Stats";
    case Mode::NoSurprises:
        return "No-Surprises (warm)";
    }
    return "?";
}

static bool isMono(Mode m) {
    return m == Mode::MonoCold || m == Mode::MonoWarm;
}
static bool isAuto(Mode m) {
    return m == Mode::AutoCold || m == Mode::AutoWarm || m == Mode::AutoWarmStats;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--cycle")
            ...
            cycleMode = CycleMode::ColdOnly;
        else if (arg == "--warm")
            cycleMode = CycleMode::WarmOnly;
    }
    ...
    auto device =
        vksdl::DeviceBuilder(instance, surface)
            .needSwapchain()
            .needDynamicRendering()
            .needSync2()
            .preferDiscreteGpu()
            .requireCoreFeatures([](VkPhysicalDeviceFeatures& f) { f.fillModeNonSolid = VK_TRUE; })
            .build()
            .value();
    ...
    std::printf("Warm cache: %s (%zu bytes)\n", haveDiskCache ? "loaded" : "empty (first run)",
                warmCache.dataSize());
    ...
    const char* fragFiles[] = {"pbr_noise.frag.spv", "voronoi.frag.spv", "marble.frag.spv",
                               "iridescent.frag.spv"};

    auto cubeVB =
        vksdl::uploadVertexBuffer(allocator, device, cubePositions, sizeof(cubePositions)).value();
    ...
    std::array<MaterialDef, TOTAL_PERMS> perms{};
    {
        ...
        for (int j = TOTAL_PERMS - 1; j > 0; --j) {
            ...
            std::swap(perms[static_cast<std::size_t>(j)], perms[static_cast<std::size_t>(k)]);
        }
    }
    ...
    auto printStats = [&]() {
        ...
        if (mode == Mode::MonoCold) {
            std::printf(
                "  Cache:  fresh empty VkPipelineCache per pipeline (no cross-pipeline reuse)\n");
        } else {
            std::printf("  Cache:  %s\n", cold ? "empty VkPipelineCache (shared across batch)"
                                               : "warm (loaded from disk)");
        }
        const char* strategy =
            isMono(mode)   ? "synchronous monolithic"
        ...
        if (isAuto(mode) && compiler) {
            ...
            std::printf("  Resolved: %s | PCCC: %s | GPL: %s | FastLink: %s\n",
                        ...
                        info.hasPCCC ? "yes" : "no", info.hasGPL ? "yes" : "no",
        ...
        if (cacheMisses > 0) {
            ...
            std::printf(
                "  Misses only (%d): mean %.2f ms | p95 %.2f ms | p99 %.2f ms | max %.2f ms\n", nm,
                missMean, missP95, missP99, missTimes.back());
        }
        ...
        for (int i = 0; i < top; ++i) {
            ...
            std::printf("    #%d  [%3d] %s/%s/%s/%s: %.2f ms %s\n", i + 1, r.permIdx,
                        shaderName(mat.shader), cullName(mat.cull), mat.blend ? "alpha" : "opaque",
                        polyName(mat.poly), r.ms, r.cacheHit ? "(cache hit)" : "");
        }
    ...
    auto resetMode = [&](Mode newMode) {
        ...
        if (cold) {
            ...
            if (warmCache.dataSize() < 100) {
                std::printf(
                    "  WARNING: warm cache is effectively empty (header only).\n"
        ...
        if (isAuto(newMode)) {
            ...
            auto cr =
                vksdl::PipelineCompiler::create(device, cacheRef, vksdl::PipelinePolicy::Auto);
            if (cr.ok()) {
                compiler.emplace(std::move(cr.value()));
                std::printf("Compiler: resolved to %s\n",
                            compiler->resolvedModel() == vksdl::PipelineModel::GPL ? "GPL"
                                                                                   : "Monolithic");
            } else {
        ...
        if (newMode == Mode::NoSurprises) {
            std::printf(
                "Strategy: FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT on every pipeline.\n"
    ...
    auto spawnMaterial = [&](int permIdx) {
        ...
        if (mode == Mode::NoSurprises) {
            ...
            if (result.ok()) {
                ...
                std::printf(
                    "[nosurp %3d] %s/%s/%s/%s/%s: %.2fms MISS (would require compilation)\n",
                    permIdx, shaderName(mat.shader), cullName(mat.cull),
                    mat.blend ? "alpha" : "opaque", polyName(mat.poly), depthCmpName(mat.depthCmp),
                    ms);
            }
            return;
        }
        ...
        if (isMono(mode)) {
            ...
            if (result.ok()) {
                ...
                std::printf("[mono %3d] %s/%s/%s/%s/%s: %.1fms%s\n", permIdx,
                            ...
                            mat.blend ? "alpha" : "opaque", polyName(mat.poly),
                            depthCmpName(mat.depthCmp), ms, hit ? " (cache hit)" : "");
                ...
                if (perPipeCache) {
                    ...
                    vkMergePipelineCaches(device.vkDevice(), warmCache.vkPipelineCache(), 1, &src);
                }
            } else {
            ...
            if (result.ok()) {
                ...
                bool isGplPath = compiler && compiler->resolvedModel() == vksdl::PipelineModel::GPL;
    ...
    if (autoCycle) {
        const char* cycleDesc =
            cycleMode == CycleMode::ColdOnly ? "cold modes only (MonoCold, AutoCold)"
            : cycleMode == CycleMode::WarmOnly
                ? "warm modes only (MonoWarm, AutoWarm, AutoWarm+Stats, NoSurprises)"
                : "all 6 modes";
        ...
        std::printf("Controls: 1=Mono(cold) 2=Mono(warm) 3=Auto(cold) 4=Auto(warm) 5=Auto+Stats "
                    "6=NoSurprises ESC=quit\n");
        std::printf("Compare: 1 vs 3 (mono vs auto, both cold) | 2 vs 4 (mono vs auto, both warm) "
                    "| 6 = prove zero compilation\n");
        std::printf(
            "CLI: --cold (cold modes only) | --warm (warm modes only) | --cycle (all 6 modes)\n");
    }
    std::printf("Note: 'cold' = empty VkPipelineCache. Internal driver caches may persist across "
                "objects/runs.\n");
    ...
    while (running) {
        ...
        int currentSector =
            static_cast<int>(std::fmod(camAngle, 2.0f * std::numbers::pi_v<float>) / sectorAngle) %
            SECTOR_COUNT;
        ...
        if (spawnNext >= TOTAL_PERMS && !statsPrinted) {
            ...
            if (autoCycle) {
                ...
                if (cycleMode == CycleMode::ColdOnly) {
                    ...
                    modeSeq = {Mode::MonoWarm, Mode::AutoWarm, Mode::AutoWarmStats,
                               Mode::NoSurprises};
                } else {
        ...
        float camDist = 14.0f;
        float eye[3] = {camDist * std::sin(camAngle), 7.0f, camDist * std::cos(camAngle)};
        ...
        for (int i = 0; i < CUBE_COUNT; ++i) {
            ...
            vksdl::Mat4 model = vksdl::mat4Mul(mat4Translate(cube.x, 0.0f, cube.z),
            ...
            bool hasSpawned =
                (matIdx >= 0 && matIdx < TOTAL_PERMS && spawned[static_cast<std::size_t>(matIdx)]);
            float alpha = 1.0f;
            if (hasSpawned)
            ...
            vkCmdPushConstants(cmd, lastBoundLayout, lastPushStages, 0, sizeof(Push), &push);
        ...
        pushQuad(overlayVerts, GRAPH_LEFT, GRAPH_TOP, GRAPH_RIGHT, GRAPH_BOTTOM, 0.08f, 0.08f,
                 0.10f, 1.0f);
        ...
        if (refY > GRAPH_TOP) { // only draw if it fits within the graph
            pushQuad(overlayVerts, GRAPH_LEFT, refY, GRAPH_RIGHT, refY + 0.003f, 0.3f, 0.3f, 0.3f);
        }
        ...
        if (mode == Mode::AutoWarmStats) {
            ...
            float dotWidth = (GRAPH_RIGHT - GRAPH_LEFT - 0.1f) / static_cast<float>(TOTAL_PERMS);
        ...
        if (overlayVertCount > 0) {
            vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, overlayPipeline.vkPipeline());
        ...
        ++frameNum;
        {
            float sinceTitleUpdate =
                std::chrono::duration<float>(renderEnd - lastTitleUpdate).count();
                ...
                if (isMono(mode) || mode == Mode::NoSurprises) {
                    ...
                    if (mode == Mode::NoSurprises) {
                        std::snprintf(buf, sizeof(buf),
                                      "Pipeline Compiler | %s | %d/%d (%d misses) | %.0f FPS "
                                      "(%.1fms) | Cache: %zu bytes",
                                      modeName(mode), spawnNext, TOTAL_PERMS, misses, fps, avg,
                                      cacheRef.dataSize());
                    } else {
                        std::snprintf(buf, sizeof(buf),
                                      "Pipeline Compiler | %s | %d/%d spawned | %.0f FPS (%.1fms) "
                                      "| Cache: %zu bytes",
                    ...
                    std::snprintf(buf, sizeof(buf),
                                  "Pipeline Compiler | %s | %d/%d spawned (%d optimized) | %.0f "
                                  "FPS (%.1fms) | Cache: %zu bytes",
