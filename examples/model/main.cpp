...
static void recordModel(VkCommandBuffer cmd, VkExtent2D extent,
    ...
    for (std::size_t i = 0; i < meshes.size(); ++i) {
        ...
        vkCmdBindIndexBuffer(cmd, meshes[i].vkIndexBuffer(), 0,
...
int main() {
    ...
    for (const auto& md : modelData.meshes) {
        ...
        meshes.push_back(
            vksdl::uploadMesh(allocator, device, md).value());
    }

    auto mvpBuffer = vksdl::BufferBuilder(allocator)
        .size(sizeof(MVP))
        .uniformBuffer()
        .build().value();
    ...
    auto pipeline = vksdl::PipelineBuilder(device)
        ...
        .vertexAttribute(0, 0, VK_FORMAT_R32G32B32_SFLOAT,
                         offsetof(vksdl::Vertex, position))
        .vertexAttribute(1, 0, VK_FORMAT_R32G32B32_SFLOAT,
                         offsetof(vksdl::Vertex, normal))
        .vertexAttribute(2, 0, VK_FORMAT_R32G32_SFLOAT,
                         offsetof(vksdl::Vertex, texCoord))
    ...
    while (running) {
        ...
        MVP mvp;
        mvp.model = glm::rotate(glm::mat4(1.0f), time,
                                glm::vec3(0.0f, 1.0f, 0.0f));
        mvp.view  = glm::lookAt(glm::vec3(0.0f, 1.5f, 3.0f),
        ...
        recordModel(frame.cmd, swapchain.extent(),
                    img.image, img.view,
