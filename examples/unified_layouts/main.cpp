...
vksdl::Image makeStorageImage(const vksdl::Allocator& allocator, std::uint32_t w, std::uint32_t h) {
...
int main() {
    ...
    auto imageA = makeStorageImage(allocator, swapchain.extent().width, swapchain.extent().height);
    auto imageB = makeStorageImage(allocator, swapchain.extent().width, swapchain.extent().height);
    ...
    while (running) {
        ...
        if (window.consumeResize()) {
            ...
            imageA =
                makeStorageImage(allocator, swapchain.extent().width, swapchain.extent().height);
            imageB =
                makeStorageImage(allocator, swapchain.extent().width, swapchain.extent().height);
            updateDescriptors();
        ...
        vksdl::blitToSwapchain(cmd, imageB, VK_IMAGE_LAYOUT_GENERAL,
        ...
        vksdl::transitionImage(
            cmd, imageB.vkImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL,
        ...
        if (frameCount % 60 == 0) {
            if (unified) {
                std::printf(
                    "[frame %d] unified=yes, 1 transition/frame (2 eliminated), 4 compute passes\n",
                    frameCount);
