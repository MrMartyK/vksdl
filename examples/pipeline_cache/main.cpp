...
int main() {
    ...
    std::printf("Pipeline cache %s (initial size: %zu bytes)\n",
                cacheLoaded ? "loaded from disk" : "created fresh", cache.dataSize());

    using Clock = std::chrono::high_resolution_clock;

    static constexpr int kPresetCount = 4;

    const char* kNames[kPresetCount] = {"Mandelbrot", "Plasma", "SDF Sphere", "Voronoi"};
    const char* kFiles[kPresetCount] = {"mandelbrot.comp.spv", "plasma.comp.spv",
                                        "sdf_sphere.comp.spv", "voronoi.comp.spv"};
    ...
    auto updateTitle = [&](int preset) {
        ...
        std::snprintf(
            ...
            kNames[preset], coldMs[preset], warmMs[preset], speedup,
    ...
    while (running) {
        ...
        if (frameNum % 60 == 0) {
            ...
            std::printf("[frame %d] preset=%s cold=%.2fms warm=%.2fms speedup=%.0fx cache=%zuB\n",
                        frameNum, kNames[currentPreset], coldMs[currentPreset],
                        warmMs[currentPreset], speedup, cache.dataSize());
        }
    }
