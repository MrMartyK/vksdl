...
static void faceNormal(float out[4],
    ...
    out[0] = nx/len; out[1] = ny/len; out[2] = nz/len; out[3] = 0.0f;
}
...
int main() {
    ...
    auto pyramidVBuf = vksdl::BufferBuilder(allocator)
        ...
        .size(sizeof(pyramidVerts)).build().value();
    if (!vksdl::uploadToBuffer(allocator, device, pyramidVBuf,
                               pyramidVerts, sizeof(pyramidVerts)).ok()) return 1;
    auto pyramidIBuf = vksdl::BufferBuilder(allocator)
        .indexBuffer().accelerationStructureInput()
        .size(sizeof(pyramidIndices)).build().value();
    if (!vksdl::uploadToBuffer(allocator, device, pyramidIBuf,
                               pyramidIndices, sizeof(pyramidIndices)).ok()) return 1;
    ...
    auto normalBuf = vksdl::uploadStorageBuffer(allocator, device,
        faceNormals, sizeof(faceNormals)).value();

    auto pyramidGeo = vksdl::BlasTriangleGeometry::fromBuffers(
        pyramidVBuf, pyramidIBuf, 5, 18, 3 * sizeof(float));
    auto pyramidBlas = vksdl::BlasBuilder(device, allocator)
        .addTriangles(pyramidGeo).preferFastTrace().build().value();
    ...
    auto sbt = vksdl::ShaderBindingTable::create(
        device, pipeline, allocator, 2, 1).value();
    ...
    while (running) {
        ...
        if (window.consumeResize()) {
            ...
            descriptors.updateImage(1, storageImage.vkImageView(),
        ...
        vksdl::blitToSwapchain(frame.cmd, storageImage,
                               VK_IMAGE_LAYOUT_GENERAL,
