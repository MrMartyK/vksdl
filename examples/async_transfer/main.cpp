...
int main() {
    ...
    while (running) {
        ...
        if (nextShape < MAX_SHAPES && (frameCount % FRAMES_PER_SHAPE) == 0) {
            ...
            auto buf = vksdl::BufferBuilder(allocator).vertexBuffer().size(sizeof(verts)).build();

            if (buf.ok()) {
                shapes[nextShape].vertexBuffer = std::move(buf.value());

                auto pend =
                    transferQ.uploadAsync(*shapes[nextShape].vertexBuffer, verts, sizeof(verts));
                ...
                if (pend.ok()) {
                    ...
                    std::printf("Shape %2d: upload started (timeline=%llu, xfer=%s)\n", nextShape,
                                ...
                                pend.value().needsOwnershipTransfer ? "cross-family"
                                                                    : "same-family");
                } else {
                    std::printf("Shape %d: uploadAsync failed: %s\n", nextShape,
                                pend.error().message.c_str());
        ...
        for (int i = 0; i < nextShape; ++i) {
            if (!shapes[i].barrierDone)
                continue;
            if (!shapes[i].vertexBuffer.has_value())
                continue;
        ...
        if ((frameCount % 60) == 0) {
            ...
            std::snprintf(title, sizeof(title),
                          ...
                          device.hasDedicatedTransfer() ? "yes" : "no", readyCount, MAX_SHAPES,
                          pendingCount);
