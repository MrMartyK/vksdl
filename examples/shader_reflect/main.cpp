...
static vksdl::Result<vksdl::Image> makeStorageImage(const vksdl::Allocator& allocator,
                                                    VkExtent2D extent) {
...
static void writeFilterDescriptors(VkDevice device, VkDescriptorSet ds, VkImageView inputView,
...
int main() {
    ...
    writeFilterDescriptors(device.vkDevice(), filterDS, imageA.vkImageView(), imageB.vkImageView());
    ...
    while (running) {
        ...
        if (window.consumeResize()) {
            ...
            genDesc.updateImage(0, imageA.vkImageView(), VK_IMAGE_LAYOUT_GENERAL);
            writeFilterDescriptors(device.vkDevice(), filterDS, imageA.vkImageView(),
                                   imageB.vkImageView());
        }
        ...
        if (frameNum % 60 == 0) {
            std::printf(
                "[frame %d] gen_scene(manual) -> edge_filter(reflected), 2 compute passes\n",
                frameNum);
        }
    }
