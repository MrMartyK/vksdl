...
DescriptorPool::DescriptorPool(DescriptorPool&& o) noexcept
    : device_(o.device_), maxSetsPerPool_(o.maxSetsPerPool_), nextPoolScale_(o.nextPoolScale_),
      allocatedSets_(o.allocatedSets_), pools_(std::move(o.pools_)) {
...
Result<DescriptorPool> DescriptorPool::create(const Device& device, std::uint32_t maxSetsPerPool) {
    DescriptorPool dp;
...
Result<std::vector<VkDescriptorSet>> DescriptorPool::allocateMany(VkDescriptorSetLayout layout,
                                                                  std::uint32_t count) {
...
Result<std::vector<VkDescriptorSet>> DescriptorPool::allocateMany(const DescriptorLayout& layout,
                                                                  std::uint32_t count) {
