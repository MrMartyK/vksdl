...
Result<QueryPool> QueryPool::create(const Device& device, VkQueryType type,
                                    std::uint32_t count) {
    if (count == 0) {
...
Result<std::vector<std::uint64_t>> QueryPool::getResults(
    ...
    VkResult vr = vkGetQueryPoolResults(
        device_, pool_, first, resultCount,
