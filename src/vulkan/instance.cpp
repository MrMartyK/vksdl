...
static const char* VkResultToString(VkResult r) {
    ...
    case VK_ERROR_DEVICE_LOST:
        return "device lost (GPU crashed or was removed)";
    case VK_ERROR_MEMORY_MAP_FAILED:
        return "memory map failed";
    case VK_ERROR_LAYER_NOT_PRESENT:
        return "requested layer not present";
    case VK_ERROR_EXTENSION_NOT_PRESENT:
        return "requested extension not present";
    case VK_ERROR_FEATURE_NOT_PRESENT:
        return "requested feature not present";
...
Instance::Instance(Instance&& o) noexcept : instance_(o.instance_), messenger_(o.messenger_) {
...
static bool LayerAvailable(const char* name, const std::vector<VkLayerProperties>& available) {
...
InstanceBuilder& InstanceBuilder::requireVulkan(std::uint32_t major, std::uint32_t minor,
                                                std::uint32_t patch) {
    ...
    for (auto* requested : extensions) {
        if (!ExtensionAvailable(requested, availableExts)) {
            return Error{"create instance", 0,
                         "Extension '" + std::string(requested) +
                             "' is not available.\n"
    ...
    if (vr != VK_SUCCESS) {
        ...
        if (vr == VK_ERROR_INCOMPATIBLE_DRIVER) {
            ...
            msg += ".\nYou requested Vulkan " + std::to_string(major) + "." +
                   std::to_string(minor) +
                   " but your driver doesn't support it.\n"
                   "Try updating your GPU drivers, or check https://vulkan.gpuinfo.org";
        }
