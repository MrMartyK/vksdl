...
PushDescriptorWriter::PushDescriptorWriter(VkPipelineLayout layout,
                                           std::uint32_t set)
    : layout_(layout), set_(set) {}

PushDescriptorWriter& PushDescriptorWriter::image(
    std::uint32_t binding, VkImageView view, VkImageLayout imageLayout,
...
PushDescriptorWriter& PushDescriptorWriter::storageImage(
    std::uint32_t binding, VkImageView view, VkImageLayout imageLayout) {
    return image(binding, view, imageLayout, VK_NULL_HANDLE,
                 VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
}

PushDescriptorWriter& PushDescriptorWriter::buffer(
    std::uint32_t binding, VkBuffer buf, VkDeviceSize size,
...
void PushDescriptorWriter::push(VkCommandBuffer cmd,
    ...
    if (!writes.empty()) {
        ...
        if (pfn) {
            pfn(cmd, bindPoint, layout_, set_,
