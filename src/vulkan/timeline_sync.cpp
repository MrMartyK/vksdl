...
TimelineSync::TimelineSync(TimelineSync&& o) noexcept
    : device_(o.device_), devicePtr_(o.devicePtr_), pool_(o.pool_), timeline_(o.timeline_),
      count_(o.count_), current_(o.current_), counter_(o.counter_), cmds_(std::move(o.cmds_)),
...
Result<TimelineSync> TimelineSync::create(const Device& device, std::uint32_t framesInFlight) {
    if (device.queueFamilies().graphics == UINT32_MAX) {
        return Error{"create timeline sync", 0, "Device has no graphics queue family"};
    }
...
Result<TimelineFrame> TimelineSync::nextFrame() {
    ...
    if (counter_ >= count_) {
        ...
        if (vr != VK_SUCCESS) {
            ...
            return Error{"wait for timeline semaphore", static_cast<std::int32_t>(vr),
...
void submitTimelineFrame(VkQueue queue, const TimelineSync& sync, const TimelineFrame& frame,
...
Result<TimelineAcquiredFrame> acquireTimelineFrame(Swapchain& swapchain, TimelineSync& sync,
                                                   const Device& device, const Window& window) {

    auto frameRes = sync.nextFrame();
    if (!frameRes.ok())
        return frameRes.error();
...
void presentTimelineFrame(const Device& device, Swapchain& swapchain, const Window& window,
                          TimelineSync& sync, const TimelineFrame& frame,
                          const SwapchainImage& image, VkPipelineStageFlags waitStage) {

    submitTimelineFrame(device.graphicsQueue(), sync, frame, image.imageReady, waitStage);

    VkResult result = swapchain.present(device.presentQueue(), image.index,
                                        sync.renderDoneSemaphore(frame.index));
