...
Image::Image(Image&& o) noexcept
    : allocator_(o.allocator_), device_(o.device_), image_(o.image_), view_(o.view_),
      allocation_(o.allocation_), format_(o.format_), extent_(o.extent_), mipLevels_(o.mipLevels_),
      samples_(o.samples_) {
...
Result<Image> ImageBuilder::build() {
    if (width_ == 0 || height_ == 0) {
        return Error{"create image", 0, "image size is 0 -- call size(width, height)"};
    }
    if (format_ == VK_FORMAT_UNDEFINED) {
    ...
    if (mipmapped_) {
        ...
        mipLevels_ =
            static_cast<std::uint32_t>(std::floor(std::log2(std::max(width_, height_)))) + 1;
    }
    ...
    VkResult vr =
        vmaCreateImage(allocator_, &imageCI, &allocCI, &img.image_, &img.allocation_, nullptr);
    if (vr != VK_SUCCESS) {
        return Error{"create image", static_cast<std::int32_t>(vr), "vmaCreateImage failed"};
    }
