...
RayTracingPipelineBuilder& RayTracingPipelineBuilder::rayGenShader(
    const std::filesystem::path& spvPath) {
    stages_.push_back({StageType::RayGen, spvPath, VK_NULL_HANDLE});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::missShader(
    const std::filesystem::path& spvPath) {
    stages_.push_back({StageType::Miss, spvPath, VK_NULL_HANDLE});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::closestHitShader(
    const std::filesystem::path& spvPath) {
    stages_.push_back({StageType::ClosestHit, spvPath, VK_NULL_HANDLE});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::anyHitShader(
    const std::filesystem::path& spvPath) {
    stages_.push_back({StageType::AnyHit, spvPath, VK_NULL_HANDLE});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::intersectionShader(
    const std::filesystem::path& spvPath) {
    stages_.push_back({StageType::Intersection, spvPath, VK_NULL_HANDLE});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::rayGenModule(
    VkShaderModule module) {
    stages_.push_back({StageType::RayGen, {}, module});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::missModule(
    VkShaderModule module) {
    stages_.push_back({StageType::Miss, {}, module});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::closestHitModule(
    VkShaderModule module) {
    stages_.push_back({StageType::ClosestHit, {}, module});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::anyHitModule(
    VkShaderModule module) {
    stages_.push_back({StageType::AnyHit, {}, module});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::intersectionModule(
    VkShaderModule module) {
    stages_.push_back({StageType::Intersection, {}, module});
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::addTrianglesHitGroup(
    std::uint32_t closestHitIndex, std::uint32_t anyHitIndex) {
    HitGroupEntry hg{};
...
RayTracingPipelineBuilder& RayTracingPipelineBuilder::addProceduralHitGroup(
    std::uint32_t intersectionIndex, std::uint32_t closestHitIndex,
...
RayTracingPipelineBuilder& RayTracingPipelineBuilder::maxRecursionDepth(
    std::uint32_t depth) {
    maxRecursion_ = depth;
    return *this;
}
...
RayTracingPipelineBuilder& RayTracingPipelineBuilder::pushConstantRange(
    VkPushConstantRange range) {
    pushConstantRanges_.push_back(range);
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::descriptorSetLayout(
    VkDescriptorSetLayout layout) {
    descriptorSetLayouts_.push_back(layout);
    return *this;
}

RayTracingPipelineBuilder& RayTracingPipelineBuilder::pipelineLayout(
    VkPipelineLayout layout) {
    externalLayout_ = layout;
    return *this;
}

Result<VkShaderModule> RayTracingPipelineBuilder::createModule(
    ...
    if (vr != VK_SUCCESS) {
        return Error{"create RT shader module",
...
Result<Pipeline> RayTracingPipelineBuilder::build() {
    ...
    for (const auto& s : stages_) {
        if (s.type == StageType::RayGen) { hasRaygen = true; break; }
    }
    if (!hasRaygen) {
    ...
    if (externalLayout_ != VK_NULL_HANDLE) {
        ...
        VkResult vr = vkCreatePipelineLayout(device_, &layoutCI, nullptr,
                                              &p.layout_);
        if (vr != VK_SUCCESS) {
            destroyCreatedModules();
            return Error{"create RT pipeline layout",
                         static_cast<std::int32_t>(vr),
    ...
    feedbackCI.pipelineStageCreationFeedbackCount =
        static_cast<std::uint32_t>(vkStages.size());
    ...
    VkResult vr = fn.createRtPipelines(
        device_, VK_NULL_HANDLE, cache_,
        1, &pipelineCI, nullptr, &p.pipeline_);
    ...
    for (std::size_t i = 0; i < vkStages.size(); ++i) {
        ...
        sf.valid      = (rtStageFeedbacks[i].flags &
                         VK_PIPELINE_CREATION_FEEDBACK_VALID_BIT) != 0;
