...
ShaderBindingTable::ShaderBindingTable(ShaderBindingTable&& o) noexcept
    : backing_(o.backing_), device_(o.device_), pfnTraceRays_(o.pfnTraceRays_), raygen_(o.raygen_),
      miss_(o.miss_), hit_(o.hit_), callable_(o.callable_) {
    o.backing_ = nullptr;
...
ShaderBindingTable& ShaderBindingTable::operator=(ShaderBindingTable&& o) noexcept {
    if (this != &o) {
        if (backing_) {
            if (backing_->buffer != VK_NULL_HANDLE) {
                vmaDestroyBuffer(backing_->allocator, backing_->buffer, backing_->allocation);
...
ShaderBindingTable::create(const Device& device, const Pipeline& rtPipeline,
                           const Allocator& allocator, std::uint32_t missCount,
                           std::uint32_t hitCount, std::uint32_t callableCount) {
    ...
    auto handleSizeAligned = static_cast<VkDeviceSize>(
        alignUp(static_cast<VkDeviceSize>(handleSize), static_cast<VkDeviceSize>(handleAlignment)));

    std::uint32_t totalGroups = 1 + missCount + hitCount + callableCount;

    std::vector<std::uint8_t> handleData(static_cast<std::size_t>(totalGroups) * handleSize);
    VkResult vr = fn.getShaderGroupHandles(device.vkDevice(), rtPipeline.vkPipeline(), 0,
                                           totalGroups, handleData.size(), handleData.data());
    if (vr != VK_SUCCESS) {
    ...
    VkDeviceSize raygenStride =
        alignUp(handleSizeAligned, static_cast<VkDeviceSize>(baseAlignment));
    VkDeviceSize raygenSize = raygenStride;

    VkDeviceSize missStride = missCount > 0 ? handleSizeAligned : 0;
    VkDeviceSize missSize = missCount > 0 ? alignUp(handleSizeAligned * missCount,
                                                    static_cast<VkDeviceSize>(baseAlignment))
                                          : 0;

    VkDeviceSize hitStride = hitCount > 0 ? handleSizeAligned : 0;
    VkDeviceSize hitSize = hitCount > 0 ? alignUp(handleSizeAligned * hitCount,
                                                  static_cast<VkDeviceSize>(baseAlignment))
                                        : 0;
    ...
    VkDeviceSize callableSize =
        callableCount > 0
            ? alignUp(callableStride * callableCount, static_cast<VkDeviceSize>(baseAlignment))
            : 0;
    ...
    vr = vmaCreateBuffer(vma, &bufCI, &allocCI, &backing->buffer, &backing->allocation, &allocInfo);
    if (vr != VK_SUCCESS) {
        delete backing;
        return Error{"create SBT", static_cast<std::int32_t>(vr), "failed to create SBT buffer"};
    }
    ...
    std::memcpy(dst + offset, handleData.data() + static_cast<std::size_t>(handleIdx) * handleSize,
                handleSize);
    handleIdx++;
    offset += raygenSize;
    ...
    sbt.callable_.deviceAddress = callableCount > 0 ? baseAddr + regionOffset : 0;
...
void ShaderBindingTable::traceRays(VkCommandBuffer cmd, std::uint32_t width, std::uint32_t height,
                                   std::uint32_t depth) const {
    pfnTraceRays_(cmd, &raygen_, &miss_, &hit_, &callable_, width, height, depth);
}
