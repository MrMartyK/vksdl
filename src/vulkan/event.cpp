...
GpuEvent::~GpuEvent() { destroy(); }

GpuEvent::GpuEvent(GpuEvent&& o) noexcept
    : device_(o.device_), event_(o.event_) {
...
Result<GpuEvent> GpuEvent::create(const Device& device) {
    ...
    if (vr != VK_SUCCESS) {
        return Error{"create gpu event", static_cast<std::int32_t>(vr),
                     "vkCreateEvent failed"};
    }

    return ev;
}

void GpuEvent::set(VkCommandBuffer cmd,
...
void GpuEvent::reset(VkCommandBuffer cmd,
