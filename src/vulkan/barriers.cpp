...
void transitionImage(VkCommandBuffer cmd, VkImage image, VkImageLayout oldLayout,
    ...
    if (srcStage == VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT ||
        ...
        std::fprintf(stderr, "[vksdl perf] transitionImage uses ALL_COMMANDS stage "
...
void transitionToPresent(VkCommandBuffer cmd, VkImage image) {
    transitionImage(
        cmd, image, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
...
void transitionToTransferSrc(VkCommandBuffer cmd, VkImage image) {
    transitionImage(cmd, image, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
...
void blitToSwapchain(VkCommandBuffer cmd, VkImage src, VkExtent2D srcExtent,
    ...
    transitionImage(cmd, src, srcCurrentLayout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, srcStage,
                    srcAccess, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_READ_BIT);

    transitionImage(cmd, dst, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                    VK_PIPELINE_STAGE_2_NONE, 0, VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                    VK_ACCESS_2_TRANSFER_WRITE_BIT);
    ...
    transitionImage(cmd, dst, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
...
void blitToSwapchain(VkCommandBuffer cmd, const Image& src, VkImageLayout srcCurrentLayout,
    ...
    blitToSwapchain(cmd, src.vkImage(), src.extent(), srcCurrentLayout, srcStage, srcAccess, dst,
                    dstExtent);
}
...
void clearImage(VkCommandBuffer cmd, VkImage image, VkClearColorValue clearValue,
    ...
    transitionImage(cmd, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, targetLayout,
                    VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT,
...
void transitionFromRTWrite(VkCommandBuffer cmd, VkImage image, VkImageLayout targetLayout,
    ...
    transitionImage(cmd, image, VK_IMAGE_LAYOUT_GENERAL, targetLayout,
                    VK_PIPELINE_STAGE_2_RAY_TRACING_SHADER_BIT_KHR,
                    VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT, dstStage, dstAccess);
}
