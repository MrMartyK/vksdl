...
Result<std::vector<std::uint32_t>> readSpv(const std::filesystem::path& path) {
    ...
    if (!file.is_open()) {
        return Error{"read SPIR-V", 0, "could not open file: " + path.string()};
    }

    auto pos = file.tellg();
    if (pos <= 0) {
        return Error{"read SPIR-V", 0, "file is empty or unreadable: " + path.string()};
    }
    auto size = static_cast<std::size_t>(pos);
    if (size % sizeof(std::uint32_t) != 0) {
        return Error{"read SPIR-V", 0, "file size is not a multiple of 4 bytes: " + path.string()};
    }
    ...
    if (code.size() >= 1 && code[0] != 0x07230203) {
        return Error{"read SPIR-V", 0, "invalid SPIR-V magic number in: " + path.string()};
    }
...
Pipeline::Pipeline(Pipeline&& o) noexcept
    : device_(o.device_), pipeline_(o.pipeline_), layout_(o.layout_), ownsLayout_(o.ownsLayout_),
      bindPoint_(o.bindPoint_), pcStages_(o.pcStages_), pcSize_(o.pcSize_),
      ownedSetLayouts_(std::move(o.ownedSetLayouts_)), stats_(std::move(o.stats_)) {
...
void Pipeline::pushConstants(VkCommandBuffer cmd, const void* data, std::uint32_t size) const {
    vkCmdPushConstants(cmd, layout_, pcStages_, 0, size, data);
}

PipelineBuilder::PipelineBuilder(const Device& device) : device_(device.vkDevice()) {}
...
PipelineBuilder& PipelineBuilder::simpleColorPipeline(const std::filesystem::path& vertSpvPath,
                                                      const std::filesystem::path& fragSpvPath,
                                                      const Swapchain& swapchain) {
    return vertexShader(vertSpvPath).fragmentShader(fragSpvPath).colorFormat(swapchain);
}
...
PipelineBuilder& PipelineBuilder::vertexBinding(std::uint32_t binding, std::uint32_t stride,
                                                VkVertexInputRate inputRate) {
    vertexBindings_.push_back({binding, stride, inputRate});
    return *this;
}

PipelineBuilder& PipelineBuilder::vertexAttribute(std::uint32_t location, std::uint32_t binding,
...
void Pipeline::setDepthTest(VkCommandBuffer cmd, bool enable, bool write, VkCompareOp op) {
...
Result<VkShaderModule> PipelineBuilder::createModule(const std::vector<std::uint32_t>& code) const {
...
Result<Pipeline> PipelineBuilder::buildWithFlags(VkPipelineCreateFlags flags) const {
    ...
    if (externalLayout_ != VK_NULL_HANDLE) {
        ...
        layoutCI.setLayoutCount = static_cast<std::uint32_t>(localDescriptorSetLayouts.size());
