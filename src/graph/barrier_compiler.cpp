...
static BarrierParams computeBarrier(const ResourceState& src,
    ...
    if (dstIsRead) {
        ...
        if (hasWrite) {
            ...
            if ((src.readStagesSinceWrite & p.dstStage) == p.dstStage
                && !layoutChange) {
                // This read's stage is fully covered by prior barriers.
                return p; // no barrier needed
            }

            if (src.readStagesSinceWrite != VK_PIPELINE_STAGE_2_NONE
                && !layoutChange) {
...
void appendImageBarrier(BarrierBatch& batch, const ImageBarrierRequest& req) {
    ...
    barrier.subresourceRange    = VkImageSubresourceRange{
        req.aspect,
        req.range.baseMipLevel,
        req.range.levelCount,
        req.range.baseArrayLayer,
