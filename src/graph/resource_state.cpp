...
bool SubresourceRange::contains(const SubresourceRange& other) const {
    return other.baseMipLevel >= baseMipLevel
        && other.mipEnd() <= mipEnd()
        && other.baseArrayLayer >= baseArrayLayer
        && other.layerEnd() <= layerEnd();
}
...
ImageSubresourceMap::ImageSubresourceMap(std::uint32_t mipLevels,
                                         std::uint32_t arrayLayers,
                                         ResourceState initialState)
{
    slices_.push_back(SubresourceSlice{
        SubresourceRange{0, mipLevels, 0, arrayLayers},
        initialState
    });
}

void ImageSubresourceMap::resetState(std::uint32_t mipLevels,
                                      std::uint32_t arrayLayers,
...
void ImageSubresourceMap::setState(const SubresourceRange& range,
                                    const ResourceState& newState)
{
    ...
    for (const auto& slice : slices_) {
        ...
        result.push_back(SubresourceSlice{
            SubresourceRange{overlapMipBase, overlapMipCount,
                             overlapLayerBase, overlapLayerCount},
            newState
        });
    }
