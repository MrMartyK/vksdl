...
PassBuilder& PassBuilder::readDepthAttachment(ResourceHandle h) {
    ResourceState state{};
    state.lastWriteStage       = VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT
...
PassBuilder& PassBuilder::setColorTarget(std::uint32_t index, ResourceHandle h,
                                          LoadOp loadOp) {
...
PassBuilder& PassBuilder::setDepthTarget(ResourceHandle h,
                                          LoadOp loadOp,
...
PassBuilder& PassBuilder::access(ResourceHandle h, AccessType type,
