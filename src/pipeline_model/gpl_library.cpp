...
GplLibrary::GplLibrary(GplLibrary&& o) noexcept : device_(o.device_), pipeline_(o.pipeline_) {
...
GplVertexInputBuilder::GplVertexInputBuilder(const Device& device) : device_(device.vkDevice()) {}

GplVertexInputBuilder& GplVertexInputBuilder::vertexBinding(std::uint32_t binding,
                                                            std::uint32_t stride,
...
GplVertexInputBuilder& GplVertexInputBuilder::vertexAttribute(std::uint32_t location,
...
Result<GplLibrary> GplVertexInputBuilder::build() const {
    ...
    vertexInput.vertexBindingDescriptionCount = static_cast<std::uint32_t>(vertexBindings_.size());
    ...
    VkResult vr =
        vkCreateGraphicsPipelines(device_, cache_, 1, &pipelineCI, nullptr, &lib.pipeline_);
    if (vr != VK_SUCCESS) {
        return Error{"create GPL vertex input library", static_cast<std::int32_t>(vr),
...
Result<GplLibrary> GplPreRasterizationBuilder::build() const {
    if (vertModule_ == VK_NULL_HANDLE) {
        return Error{"create GPL pre-rasterization library", 0, "no vertex shader module set"};
    }
    ...
    VkResult vr =
        vkCreateGraphicsPipelines(device_, cache_, 1, &pipelineCI, nullptr, &lib.pipeline_);
    if (vr != VK_SUCCESS) {
        return Error{"create GPL pre-rasterization library", static_cast<std::int32_t>(vr),
...
GplFragmentShaderBuilder& GplFragmentShaderBuilder::depthTest(bool enable, bool write,
                                                              VkCompareOp op) {
...
GplFragmentShaderBuilder& GplFragmentShaderBuilder::sampleShading(bool enable, float min) {
...
Result<GplLibrary> GplFragmentShaderBuilder::build() const {
    if (fragModule_ == VK_NULL_HANDLE) {
        return Error{"create GPL fragment shader library", 0, "no fragment shader module set"};
    }
    if (layout_ == VK_NULL_HANDLE) {
    ...
    VkResult vr =
        vkCreateGraphicsPipelines(device_, cache_, 1, &pipelineCI, nullptr, &lib.pipeline_);
    if (vr != VK_SUCCESS) {
        return Error{"create GPL fragment shader library", static_cast<std::int32_t>(vr),
...
Result<GplLibrary> GplFragmentOutputBuilder::build() const {
    if (colorFormat_ == VK_FORMAT_UNDEFINED) {
        return Error{"create GPL fragment output library", 0, "no color format set"};
    }
    ...
    VkResult vr =
        vkCreateGraphicsPipelines(device_, cache_, 1, &pipelineCI, nullptr, &lib.pipeline_);
    if (vr != VK_SUCCESS) {
        return Error{"create GPL fragment output library", static_cast<std::int32_t>(vr),
...
Result<VkPipeline> linkGplPipeline(const Device& device, const GplLibrary& vi, const GplLibrary& pr,
                                   const GplLibrary& fs, const GplLibrary& fo,
                                   VkPipelineLayout layout, VkPipelineCache cache, bool optimized) {

    VkPipeline libraries[4] = {vi.vkPipeline(), pr.vkPipeline(), fs.vkPipeline(), fo.vkPipeline()};
    ...
    VkResult vr = vkCreateGraphicsPipelines(vkDev, cache, 1, &pipelineCI, nullptr, &pipeline);
    if (vr != VK_SUCCESS) {
